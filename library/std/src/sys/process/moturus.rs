...
use crate::sys::io_pipe::IoPipe;
use crate::sys::map_moturus_error;
...
impl Command {
    ...
    pub fn spawn(
        ...
        let (handle, stdin, stdout, stderr) =
            moto_rt::process::spawn(args).map_err(map_moturus_error)?;
...
impl Process {
    ...
    pub fn kill(&mut self) -> io::Result<()> {
        match moto_rt::process::kill(self.handle) {
            moto_rt::E_OK => Ok(()),
            err => Err(map_moturus_error(err)),
        }
    }

    pub fn wait(&mut self) -> io::Result<ExitStatus> {
        moto_rt::process::wait(self.handle).map(|c| ExitStatus(c)).map_err(map_moturus_error)
    }
